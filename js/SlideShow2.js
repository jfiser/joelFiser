...
show a grid - but could be anything). set "gridIcon" in the JSON file to the url of the image / svg.
...
- Update (3/20/2015) - moved caption and arrows outside of slideShowHolder and now
parented by myParent (the main DIV). Now you can have 2 captions, and they can appear anywhere.
...
function SlideShow2(_myParentId, _imagesJsonUrl){
    ...
    console.log("TTTThis: %o", this);
    console.log("gasAudio: " + this.hasAudioPlayer);
    console.log("gridIcon: " + this.SMALL_SCREEN);
...
SlideShow2.prototype.windowResize = function(_this){
...
SlideShow2.prototype.setKeyPress = function(){
    var self = this;
...
SlideShow2.prototype.arrowResize = function(_this){
    if(this.stackImageArrowsText != undefined 
                        && this.stackImageArrowsText != ""
                        && this.stackImageArrowsText != "false"){
...
SlideShow2.prototype.getImageData = function(_imagesJsonUrl){
	var self = this;
	var slide, numToLoad;
	...
	$.getJSON(_imagesJsonUrl, function(_dataObj){
            ...
            for(var i = 0; i < numToLoad; i++){
                slide = new Slide2(self, self.imageArr[i], 
                                    self.slidesHolderId, 
                                    self.imageArr[i].clickAction_slideShow, 
                                    i);
            ...
            self.captionBar = new CaptionBar2(self, self.slideShowHolder, 
                                    self.imageArr.length, self.showCloseBtn,
                                    self.captionBarHolderId);
            self.captionBar2 = new CaptionBar2(self, self.slideShowHolder, 
                                    self.imageArr.length, self.showCloseBtn,
                                    self.captionBarHolderId2);
            if(self.usePageCount == "true"){
                self.pageCount = new PageCount(self, self.pageCountHolderId,
                                    self.imageArr.length);
            }
            if(self.hasAudioPlayer){
                self.audioPlayer = new AudioPlayer(self, self.audioPlayerHolderId);
            }
            ...
            if(!self.hasCoverSlide){
                ...
                self.captionBar2.updateCaptionBar(0, self.slideArr[0].caption2);
            }
			if(this.hasPageCount){
				self.pageCount.updatePageCount(0);
			}
        });
}
SlideShow2.prototype.loadRestOfImages = function(){
    var slide;
    ...
    for(var i = this.numImagesToLoadAtStart; i < this.imageArr.length; i++){
        ...
        slide = new Slide2(this, 
                            this.imageArr[i], 
                            this.slidesHolderId, 
                            this.imageArr[i].clickAction_slideShow, 
                            i);
        this.slideArr.push(slide);
    }
    this.allImagesLoaded = true;
}
SlideShow2.prototype.addSlideShowHolder = function(){
	...
	//$('#' + this.slideSxhowHolderId).append('<div class="arrowHolder" id="' + this.arrowHolderId + '"></div>');
	$(this.myParent).append('<div class="arrowHolder" id="' + this.arrowHolderId + '"></div>');
    
    this.captionBarHolderId = "captionBarHolder_" + Math.floor(Math.random() * 100000);
    this.captionBarHolderId2 = "captionBarHolder2_" + Math.floor(Math.random() * 100000);
	
	$(this.myParent).append('<div class="captionBarHolder" id="' + this.captionBarHolderId + '"></div>');
	$(this.myParent).append('<div class="captionBarHolder2" id="' + this.captionBarHolderId2 + '"></div>');
    
    this.pageCountHolderId = "pageCountHolder_" + Math.floor(Math.random() * 100000);
    $(this.myParent).append('<div class="pageCountHolder" id="' + this.pageCountHolderId + '"></div>');
    
    
}
SlideShow2.prototype.addArrows = function(_hasCoverSlide){
	...
	$('#' + this.arrowHolderId).append('<img class="arrowLeft" id="' + this.arrowLeftId 
                ...
                + '"/>');
	var self = this;
	$('#' + this.arrowLeftId).click(function(evt){
        evt.stopPropagation();
        self.setPrevSlide(self);
        if(self.swipeType === "fade"){
            self.prevSlideFade(self);
	...
	$('#' + this.arrowHolderId).append('<img class="arrowRight" id="' + this.arrowRightId 
                ...
                + ($(window).width() < this.SMALL_SCREEN && this.rightArrowUrl_mobile != undefined 
                                    ...
                                    : this.rightArrowUrl)
                + '"/>');
	$('#' + this.arrowRightId).click(function(evt){
        evt.stopPropagation();
        self.setNextSlide(self);
        if(self.swipeType === "fade"){
            self.nextSlideFade(self);
...
SlideShow2.prototype.showArrows = function(){
	$('#' + this.arrowRightId).show();
	$('#' + this.arrowLeftId).show();
}
SlideShow2.prototype.checkCoverSlide = function(){
...
SlideShow2.prototype.deepLinkToSlide = function(_indx){
	console.log("deepLinkToSlide: " + _indx);
    this.topZindex++;
    this.curIndx = _indx;
    ...
    this.captionBar.updateCaptionBar(this.curIndx, 
                                     this.slideArr[this.curIndx].caption);
    this.captionBar2.updateCaptionBar(this.curIndx, 
                                      this.slideArr[this.curIndx].caption2);
    
    if(this.hasAudioPlayer){
        this.audioPlayer.setNextAudio(this.slideArr[this.curIndx].audio);
    }
}
SlideShow2.prototype.setNextSlide = function(_this){
...
SlideShow2.prototype.nextSlide = function(_this){
    ...
    this.captionBar2.updateCaptionBar(this.curIndx,         this.slideArr[this.curIndx].caption2);
    if(this.hasPageCount){
        this.pageCount.updatePageCount(this.curIndx);
    }
    if(this.hasAudioPlayer){
        this.audioPlayer.setNextAudio(this.slideArr[this.curIndx].audio);
    }
}
SlideShow2.prototype.nextSlideFade = function(_this){
	//console.log("nextSlide: %o", this.topSlideHolder);
...
SlideShow2.prototype.setPrevSlide = function(_this){
	if(this.checkCoverSlide()){
		return;
	}
...
SlideShow2.prototype.prevSlide = function(_this){
...
SlideShow2.prototype.prevSlideFade = function(_this){
	TweenLite.to(this.topSlideHolder, .7, {opacity:0, ease:Expo.easeOut});
    //TweenLite.to(this.newSlideHolder, .7, {opacity:1});
    // Now set curIndx - because we're changing slides
    if(this.curIndx == 0){
...
SlideShow2.prototype.hideSlideShow = function(_this){
    if(this.turnOffScroll){
        $('body').removeClass('stop-scrolling');
    }
    if(this.hasAudioPlayer){
        this.audioPlayer.pauseAudio();
    }
    if(this.scrollDownToHideAd && this.prevScrollTop != undefined){
        window.scrollTo(0, this.prevScrollTop);
    }
    $(this.myParent).hide();
}
SlideShow2.prototype.showSlideShow = function(_prevScrollTop){
    if(this.turnOffScroll){
        $('body').addClass('stop-scrolling');
    }
    $(this.myParent).show();
    if(_prevScrollTop != undefined && _prevScrollTop >= 0){
        this.prevScrollTop = _prevScrollTop;
    }
    else{
...
function Slide2(_slideShow, _slideObj, _appendToId, _clickAction, _indx){
    ...
    this.setImageClickUrl(this.clickAction);
...
Slide2.prototype.setImageClickUrl = function(){
    if(this.clickAction != undefined && this.clickAction != ""){
        var self = this;
        $(this.imageHolder).click(function(evt){
            evt.stopPropagation();
            window.location.href = self.clickAction;
        });
    }
    else{ 
        // if no clickAction set, stopPropagation because the parent  
        // might have a click action - such as with a modal
        $(this.imageHolder).click(function(evt){
            evt.stopPropagation();
        });
    }
...
Slide2.prototype.setSwipeFade = function(_slideHolder){
...
// We have two types of switching out images - sliding or fading
Slide2.prototype.setSwipeSlide = function(_slideHolder){
    //var el = document.getElementById(this.slideId);
    var slideShow = this.slideShow;
    var cur_x = 0;
    var $slideHolder = $("#" + this.slideId);
...
Slide2.prototype.chooseImage = function(_slideObj){
...
Slide2.prototype.setCoverSlide = function(_coverSlideObj){
...
Slide2.prototype.loadImage = function(){
	this.img = new Image();
	var self = this;
...
Slide2.prototype.sizeImage = function(){
...
Slide2.prototype.hideSlide = function(_topZindex){
	$(this.slideHolder).css("visibility", "hidden");
}
Slide2.prototype.showSlide = function(_topZindex){
...
Slide2.prototype.setCrop = function(_crop){
	switch(_crop){
...
function CaptionBar2(_slideShow, _slideShowHolder, _numSlides, _showCloseBtn, _captionBarHolderId){
    this.slideShow = _slideShow;
...
CaptionBar2.prototype.hideCaptionBar = function(){
    $("#" + this.captionBarHolderId).hide();
}
CaptionBar2.prototype.updateCaptionBar = function(_curIndx, _caption){
    $("#" + this.captionBarHolderId).show();
    //$("#" + this.pageCountId).html((_curIndx+1) + "/" + this.numSlides);
    $("#" + this.captionTextId).html(_caption);
}
CaptionBar2.prototype.addCaptionText = function(_slideShowHolder){
	this.captionTextId = "captionText_" + Math.floor(Math.random() * 100000);
	$('#' + this.captionBarHolderId).append('<p class="captionText" id="' +  this.captionTextId + '">' + "" + '</p>');
	$("#" + this.captionTextId).click(function(evt){
        evt.stopPropagation();
    });
	//$('#' + this.captionTextId).fitText(4.2, { minFontSize: '13px', maxFontSize: '17px' });
	this.resize();
}
CaptionBar2.prototype.addOpenCloseBtns = function(_slideShowHolder){
	var self = this;
	// open Arrow
...
CaptionBar2.prototype.closeBtnClick = function(_this){
...
CaptionBar2.prototype.openBtnClick = function(_this){
	$('#' + _this.openBtnId).hide();
	TweenLite.to($('#' + _this.captionBarHolderId), .3, {bottom:"0px", ease:Expo.easeOut});
}

CaptionBar2.prototype.resize = function(){
    return;    
}
/************** PageCount class **********/
function PageCount(_slideShow, _pageCountHolderId, _numSlides){
	this.slideShow = _slideShow;
    this.pageCountHolderId = _pageCountHolderId;
    this.numSlides = _numSlides;
    this.addPageCount();
}
PageCount.prototype.addPageCount = function(){
    this.pageCountId = "pageCount_" + Math.floor(Math.random() * 100000);
    console.log("usePageCount: " + this.slideShow.usePageCount);
    $('#' + this.pageCountHolderId).append(
        '<p class="pageCount" id="' +  this.pageCountId + '">' 
            + '1' + '/' + this.numSlides + '</p>'
            + '<img class="gridIcon" src="' 
            + this.slideShow.gridIcon + '"/>'
    );
    var self = this;
    $(".gridIcon").click(function(){
        self.slideShow.hideSlideShow();
    });
}
PageCount.prototype.updatePageCount = function(_curIndx){
    console.log("updatepagecount: " + _curIndx);
    $("#" + this.pageCountId).html((_curIndx+1) + "/" + this.numSlides);

}
/*********** AudioPayer *****************/
function AudioPlayer(_slideShow, _audioPlayerHolderId){
    var self = this;
    this.slideShow = _slideShow;
    this.audioPlayerHolderId = _audioPlayerHolderId;
    this.addAudioPlayer();
    this.setControls();
	window.addEventListener('resize', function(){self.resize(self);});
    //this.setNextAudio("http://ngm-ipad.nationalgeographic.com/interactives/2015-01/healing-soldiers/audio/Chris_McNair.mp3");
}
AudioPlayer.prototype.resize = function(_this){
    $("#" + _this.progressBarId).css("width", 
            $("#" + _this.audioPlayerHolderId).width() 
                        - $("#" + _this.playButtonId).width() + "px");
}
AudioPlayer.prototype.setNextAudio = function(_src){
    //$("audio").attr("src", _src);
    $("#" + this.audioId).attr("src", _src);
    this.progress.style.width = "0%";
    this.showPlayButton();
    
    var audioElem = document.getElementById(this.audioId);
    audioElem.addEventListener('loadedmetadata', 
                    function(evt){
                        var audioLength = parseInt(evt.target.duration);
                        $('#ss_audioDuration').text("0:" + audioLength);
                    });
}
AudioPlayer.prototype.addAudioPlayer = function(){
    this.audioPlayerHolderId = "audioPlayerHolder_" 
                        + Math.floor(Math.random() * 100000);
    $(this.slideShow.myParent).append('<div class="audioPlayerHolder" id="' 
                        + this.audioPlayerHolderId + '"></div>');
    
    this.audioId = "audio_" + Math.floor(Math.random() * 100000);
    this.progressBarId = "progressBar_" + Math.floor(Math.random() * 100000);
    this.progressId = "progress_" + Math.floor(Math.random() * 100000);
    this.audioPlayerId = "audioPlayer_" + Math.floor(Math.random() * 100000);
    this.playButtonId = "playButton_" + Math.floor(Math.random() * 100000);
    
    $('#' + this.audioPlayerHolderId).append(
        '<div class="audioPlayer">'
            + '<audio id="' + this.audioId + '"'
                            + ' class="audioFile" controls="controls"></audio>'
            + '<img id="' + this.playButtonId + '"'
                            + ' class="playButton" src="img/audio/audio_play.svg"/>'
            + '<img class="pauseButton" src="img/audio/audio_pause.svg"/>'
            + '<div id="' + this.progressBarId + '" class="progressBar">'
                + '<span id="' + this.progressId + '" class="progress"></span>'
            + '</div>'
            + '<div id="ss_audioDuration"></div>'
        + '</div>'
    );
}
AudioPlayer.prototype.setControls = function(){
    var self = this;
    ...
    function updateProgress(evt){
        var value = 0;
        console.log("evt: %o", self.audio.currentTime);
        if(self.audio.currentTime > 0){
            value = Math.floor((100 / self.audio.duration) 
...
AudioPlayer.prototype.showPlayButton = function(){
    $(this.audio).siblings('.playButton').show();
    $(this.audio).siblings('.pauseButton').hide();
}
AudioPlayer.prototype.showPauseButton = function(){
    $(this.audio).siblings('.playButton').hide();
    $(this.audio).siblings('.pauseButton').show();
}
/*AudioPlayer.prototype.updateProgress = function(evt){
    var value = 0;
    console.log("zevt: " + evt.target.audio.currentTime);
    if(evt.target.audio.currentTime > 0){
        value = Math.floor((100 / evt.target.audio.duration) 
                           * evt.target.audio.currentTime);
